/**
 * JJ 经典斗地主牌型规则 + 连炸
 * 牌型：单/对/三/三带/顺/连对/飞机/四带/炸弹/连炸/火箭
 */
...
export const HandType = {
  ...
  BOMB: 'bomb',
  CHAIN_BOMB: 'chain_bomb',     // 连炸 ≥2 连续炸弹
  ROCKET: 'rocket',
};

export { evaluateChainBomb, ChainBombType };
...
export function parseHand(cards) {
  ...
  if (n === 6 && byCount(4).length === 1) {
    ...
    if (kickers.length === 2) {
      ...
      if (!(kickerRanks.has(16) && kickerRanks.has(17))) {
...
export function canBeat(prev, next) {
  ...
  return canBeatWithChainBombRules(prev, next, (a, b) => {
    // 经典同型（不含炸体系，炸已在 chain 规则处理）
...
export function findBeatingHands(hand, prevHand) {
  ...
  // 炸弹 / 连炸 / 火箭（除非 prev 是火箭）
  if (prev.type !== HandType.ROCKET) {
    const minBombW = prev.type === HandType.BOMB ? prev.weight : 0;
    results.push(...findBombs(cards, minBombW));
    results.push(...findChainBombs(cards, prev));
...
function enumerateAllPlays(cards) {
  ...
  plays.push(...findChainBombs(cards, null));
  const rocket = findRocket(cards);
...
function bombPriority(p) {
  if (p.type === HandType.ROCKET) return 3;
  if (p.type === HandType.CHAIN_BOMB || p.type === 'chain_bomb') return 2;
...
/** 枚举手中可组成的连炸（可压 prev 的由 canBeat 过滤） */
function findChainBombs(cards, prev) {
  const groups = groupByRank(cards);
  const fours = [...groups.keys()]
    .filter((r) => r >= 3 && r <= 14 && groups.get(r).length >= 4)
    .sort((a, b) => a - b);
  if (fours.length < 2) return [];
  const results = [];
  for (let len = 2; len <= fours.length; len++) {
    for (let i = 0; i + len <= fours.length; i++) {
      const seg = fours.slice(i, i + len);
      let ok = true;
      ...
      if (!ok) continue;
      // 相对 prev 剪枝
      if (prev?.type === HandType.CHAIN_BOMB) {
        if (len < prev.length) continue;
        if (len === prev.length && seg[seg.length - 1] <= prev.weight) continue;
      }
      const combo = [];
      for (const r of seg) combo.push(...groups.get(r).slice(0, 4));
      results.push(combo);
    }
  }
  return results;
