/**
 * play9gd1b — version visible / room-api errors distinguishable + full regression
 */
...
test('lobby + health show version/cache stamp', () => {
  ...
  assert.equal(health.cache, 'play9gd1b');
  assert.equal(health.version, 'play9gd1b');
  assert.match(formatLobbyVersionLabel(), /play9gd1b/);
  ...
  assert.match(colySrv, /version:.*play9gd1b|TEA_PARLOR_VERSION/);
});
