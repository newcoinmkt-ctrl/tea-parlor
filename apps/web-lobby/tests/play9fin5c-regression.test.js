/**
 * play9ui2b — version visible / room-api errors distinguishable + full regression
 */
...
test('lobby + health show version/cache stamp', () => {
  ...
  assert.equal(health.cache, 'play9ui2b');
  assert.equal(health.version, 'play9ui2b');
  assert.match(formatLobbyVersionLabel(), /play9ui2b/);
  ...
  assert.match(colySrv, /version:.*play9ui2b|TEA_PARLOR_VERSION/);
});
