/**
 * play9ui2a — version visible / room-api errors distinguishable + full regression
 */
...
test('lobby + health show version/cache stamp', () => {
  ...
  assert.equal(health.cache, 'play9ui2a');
  assert.equal(health.version, 'play9ui2a');
  assert.match(formatLobbyVersionLabel(), /play9ui2a/);
  ...
  assert.match(colySrv, /version:.*play9ui2a|TEA_PARLOR_VERSION/);
});
