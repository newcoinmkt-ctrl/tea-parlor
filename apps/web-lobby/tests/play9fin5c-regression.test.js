/**
 * play9fin7a — version visible / room-api errors distinguishable + full regression
 */
...
test('lobby + health show version/cache stamp', () => {
  ...
  assert.equal(health.cache, 'play9fin7a');
  assert.equal(health.version, 'play9fin7a');
  assert.match(formatLobbyVersionLabel(), /play9fin7a/);
  ...
  assert.match(colySrv, /version:.*play9fin7a|TEA_PARLOR_VERSION/);
});
