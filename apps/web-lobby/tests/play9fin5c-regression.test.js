/**
 * play9gd1d — version visible / room-api errors distinguishable + full regression
 */
...
test('lobby + health show version/cache stamp', () => {
  ...
  assert.equal(health.cache, 'play9gd1d');
  assert.equal(health.version, 'play9gd1d');
  assert.match(formatLobbyVersionLabel(), /play9gd1d/);
  ...
  assert.match(colySrv, /version:.*play9gd1d|TEA_PARLOR_VERSION/);
});
