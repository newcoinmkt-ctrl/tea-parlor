/**
 * play9fin5c — version visible / room-api errors distinguishable + full regression
 */
...
test('cache play9fin5c', () => {
  assert.match(html, /app\.js\?v=play9fin5c/);
  assert.match(app, /\?v=play9fin5c/);
  assert.equal(CACHE_STAMP, 'play9fin5c');
  assert.equal(BUILD_VERSION, 'play9fin5c');
...
test('lobby + health show version/cache stamp', () => {
  ...
  assert.equal(health.cache, 'play9fin5c');
  assert.equal(health.version, 'play9fin5c');
  assert.match(formatLobbyVersionLabel(), /play9fin5c/);
  ...
  assert.match(colySrv, /version:.*play9fin5c|TEA_PARLOR_VERSION/);
});
...
test('docs/qa play9fin5c present', () => {
  assert.equal(existsSync(join(repo, 'docs/qa/play9fin5c/README.md')), true);
  assert.equal(existsSync(join(repo, 'docs/qa/play9fin5c/report.json')), true);
});
