/**
 * play9ui1a — version visible / room-api errors distinguishable + full regression
 */
...
test('cache play9ui1a', () => {
  ...
  assert.equal(CACHE_STAMP, 'play9ui1a');
  assert.equal(BUILD_VERSION, 'play9ui1a');
...
test('lobby + health show version/cache stamp', () => {
  ...
  assert.equal(health.cache, 'play9ui1a');
  assert.equal(health.version, 'play9ui1a');
  assert.match(formatLobbyVersionLabel(), /play9ui1a/);
  ...
  assert.match(colySrv, /version:.*play9ui1a|TEA_PARLOR_VERSION/);
});
