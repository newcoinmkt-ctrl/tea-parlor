...
test('play9fin2a cache bust on CSS + app.js', () => {
  assert.match(html, /app\.js\?v=play9fin2a/);
  assert.match(html, /hand-fit\.css\?v=play9fin2a/);
  assert.match(html, /table-landscape\.css\?v=play9fin2a/);
  assert.match(html, /jj-table\.css\?v=play9fin2a/);
  assert.match(html, /jj-mahjong\.css\?v=play9fin2a/);
  assert.match(html, /niuniu\.css\?v=play9fin2a/);
...
test('table-orient pins JJ zones absolute-to-stage', () => {
  ...
  assert.match(orientSrc, /padding-bottom.*safe-bottom|safePad/);
...
test('play9nn1 mahjong+niuniu adaptive stage (no rotate)', () => {
  assert.match(orientSrc, /isMjTableActive/);
  assert.match(orientSrc, /isNnTableActive/);
  assert.match(orientSrc, /table-stage-nn/);
...
test('play9fin1c prefers viewportStableHeight over innerHeight', () => {
  assert.match(orientSrc, /resolveTelegramViewportHeight/);
