/**
 * play9ui2a — lobby activity / daily supply entry
...
test('cache play9ui2a', () => {
  ...
  assert.match(html, /lobby-ia\.css\?v=play9ui2a/);
...
test('rewards are chips only — non-withdrawable; no Stars/chain top-up on activity', () => {
  ...
  assert.match(lia, /play9ui2a/);
  assert.match(lia, /activity-entry-btn/);
  assert.match(lia, /activity-page/);
});

test('docs/qa play9ui2a present', () => {
  assert.equal(existsSync(join(root, '../../docs/qa/play9ui2a/README.md')), true);
  assert.equal(existsSync(join(root, '../../docs/qa/play9ui2a/report.json')), true);
});
