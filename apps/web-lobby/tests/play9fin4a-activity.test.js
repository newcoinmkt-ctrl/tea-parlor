/**
 * play9fin7c — lobby activity / daily supply entry
...
test('rewards are chips only — non-withdrawable; no Stars/chain top-up on activity', () => {
  ...
  assert.match(lia, /play9fin7c/);
  assert.match(lia, /activity-entry-btn/);
  assert.match(lia, /activity-page/);
});

test('docs/qa play9fin7c present', () => {
  assert.equal(existsSync(join(root, '../../docs/qa/play9fin7c/README.md')), true);
  assert.equal(existsSync(join(root, '../../docs/qa/play9fin7c/report.json')), true);
});
