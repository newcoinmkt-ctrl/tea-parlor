/**
 * play9fin5a — lobby activity / daily supply entry
...
test('rewards are chips only — non-withdrawable; no Stars/chain top-up on activity', () => {
  ...
  assert.match(lia, /play9fin5a/);
  assert.match(lia, /activity-entry-btn/);
  assert.match(lia, /activity-page/);
});

test('docs/qa play9fin5a present', () => {
  assert.equal(existsSync(join(root, '../../docs/qa/play9fin5a/README.md')), true);
  assert.equal(existsSync(join(root, '../../docs/qa/play9fin5a/report.json')), true);
});
