/**
 * play9ui1d — lobby activity / daily supply entry
...
test('cache play9ui1d', () => {
  ...
  assert.match(html, /lobby-ia\.css\?v=play9ui1d/);
...
test('rewards are chips only — non-withdrawable; no Stars/chain top-up on activity', () => {
  ...
  assert.match(lia, /play9ui1d/);
  assert.match(lia, /activity-entry-btn/);
  assert.match(lia, /activity-page/);
});

test('docs/qa play9ui1d present', () => {
  assert.equal(existsSync(join(root, '../../docs/qa/play9ui1d/README.md')), true);
  assert.equal(existsSync(join(root, '../../docs/qa/play9ui1d/report.json')), true);
});
