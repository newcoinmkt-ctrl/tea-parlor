/**
 * play9ui1e — lobby activity / daily supply entry
...
test('cache play9ui1e', () => {
  ...
  assert.match(html, /lobby-ia\.css\?v=play9ui1e/);
...
test('rewards are chips only — non-withdrawable; no Stars/chain top-up on activity', () => {
  ...
  assert.match(lia, /play9ui1e/);
  assert.match(lia, /activity-entry-btn/);
  assert.match(lia, /activity-page/);
});

test('docs/qa play9ui1e present', () => {
  assert.equal(existsSync(join(root, '../../docs/qa/play9ui1e/README.md')), true);
  assert.equal(existsSync(join(root, '../../docs/qa/play9ui1e/report.json')), true);
});
