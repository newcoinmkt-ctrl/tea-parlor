/**
 * play9fin6b — lobby activity / daily supply entry
...
test('cache play9fin6b', () => {
  ...
  assert.match(html, /lobby-ia\.css\?v=play9fin6b/);
...
test('rewards are chips only — non-withdrawable; no Stars/chain top-up on activity', () => {
  ...
  assert.match(lia, /play9fin6b/);
  assert.match(lia, /activity-entry-btn/);
  assert.match(lia, /activity-page/);
});

test('docs/qa play9fin6b present', () => {
  assert.equal(existsSync(join(root, '../../docs/qa/play9fin6b/README.md')), true);
  assert.equal(existsSync(join(root, '../../docs/qa/play9fin6b/report.json')), true);
});
