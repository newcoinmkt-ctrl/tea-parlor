/**
 * play9ui1c — lobby activity / daily supply entry
...
test('cache play9ui1c', () => {
  ...
  assert.match(html, /lobby-ia\.css\?v=play9ui1c/);
...
test('rewards are chips only — non-withdrawable; no Stars/chain top-up on activity', () => {
  ...
  assert.match(lia, /play9ui1c/);
  assert.match(lia, /activity-entry-btn/);
  assert.match(lia, /activity-page/);
});

test('docs/qa play9ui1c present', () => {
  assert.equal(existsSync(join(root, '../../docs/qa/play9ui1c/README.md')), true);
  assert.equal(existsSync(join(root, '../../docs/qa/play9ui1c/report.json')), true);
});
