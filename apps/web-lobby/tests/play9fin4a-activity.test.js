/**
 * play9fin6a — lobby activity / daily supply entry
...
test('cache play9fin6a', () => {
  ...
  assert.match(html, /lobby-ia\.css\?v=play9fin6a/);
...
test('rewards are chips only — non-withdrawable; no Stars/chain top-up on activity', () => {
  ...
  assert.match(lia, /play9fin6a/);
  assert.match(lia, /activity-entry-btn/);
  assert.match(lia, /activity-page/);
});

test('docs/qa play9fin6a present', () => {
  assert.equal(existsSync(join(root, '../../docs/qa/play9fin6a/README.md')), true);
  assert.equal(existsSync(join(root, '../../docs/qa/play9fin6a/report.json')), true);
});
