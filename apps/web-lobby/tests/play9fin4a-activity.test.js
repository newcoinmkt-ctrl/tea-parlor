/**
 * play9gd1d — lobby activity / daily supply entry
...
test('cache play9gd1d', () => {
  ...
  assert.match(html, /lobby-ia\.css\?v=play9gd1d/);
...
test('rewards are chips only — non-withdrawable; no Stars/chain top-up on activity', () => {
  ...
  assert.match(lia, /play9gd1d/);
  assert.match(lia, /activity-entry-btn/);
  assert.match(lia, /activity-page/);
});

test('docs/qa play9gd1d present', () => {
  assert.equal(existsSync(join(root, '../../docs/qa/play9gd1d/README.md')), true);
  assert.equal(existsSync(join(root, '../../docs/qa/play9gd1d/report.json')), true);
});
