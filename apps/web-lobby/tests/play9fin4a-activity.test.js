/**
 * play9ui2d — lobby activity / daily supply entry
...
test('cache play9ui2d', () => {
  ...
  assert.match(html, /lobby-ia\.css\?v=play9ui2d/);
...
test('rewards are chips only — non-withdrawable; no Stars/chain top-up on activity', () => {
  ...
  assert.match(lia, /play9ui2d/);
  assert.match(lia, /activity-entry-btn/);
  assert.match(lia, /activity-page/);
});

test('docs/qa play9ui2d present', () => {
  assert.equal(existsSync(join(root, '../../docs/qa/play9ui2d/README.md')), true);
  assert.equal(existsSync(join(root, '../../docs/qa/play9ui2d/report.json')), true);
});
