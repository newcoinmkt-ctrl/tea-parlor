/**
 * play9ui1b — lobby activity / daily supply entry
...
test('cache play9ui1b', () => {
  ...
  assert.match(html, /lobby-ia\.css\?v=play9ui1b/);
...
test('rewards are chips only — non-withdrawable; no Stars/chain top-up on activity', () => {
  ...
  assert.match(lia, /play9ui1b/);
  assert.match(lia, /activity-entry-btn/);
  assert.match(lia, /activity-page/);
});

test('docs/qa play9ui1b present', () => {
  assert.equal(existsSync(join(root, '../../docs/qa/play9ui1b/README.md')), true);
  assert.equal(existsSync(join(root, '../../docs/qa/play9ui1b/report.json')), true);
});
