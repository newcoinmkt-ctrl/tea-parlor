/**
 * play9fin5b — lobby activity / daily supply entry
...
test('rewards are chips only — non-withdrawable; no Stars/chain top-up on activity', () => {
  ...
  assert.match(lia, /play9fin5b/);
  assert.match(lia, /activity-entry-btn/);
  assert.match(lia, /activity-page/);
});

test('docs/qa play9fin5b present', () => {
  assert.equal(existsSync(join(root, '../../docs/qa/play9fin5b/README.md')), true);
  assert.equal(existsSync(join(root, '../../docs/qa/play9fin5b/report.json')), true);
});
