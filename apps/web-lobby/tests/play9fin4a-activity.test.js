/**
 * play9fin5c — lobby activity / daily supply entry
...
test('cache play9fin5c', () => {
  ...
  assert.match(html, /lobby-ia\.css\?v=play9fin5c/);
...
test('rewards are chips only — non-withdrawable; no Stars/chain top-up on activity', () => {
  ...
  assert.match(lia, /play9fin5c/);
  assert.match(lia, /activity-entry-btn/);
  assert.match(lia, /activity-page/);
});

test('docs/qa play9fin5c present', () => {
  assert.equal(existsSync(join(root, '../../docs/qa/play9fin5c/README.md')), true);
  assert.equal(existsSync(join(root, '../../docs/qa/play9fin5c/report.json')), true);
});
