/**
 * play9fin4c — lobby activity / daily supply entry
...
test('cache play9fin4c', () => {
  ...
  assert.match(html, /lobby-ia\.css\?v=play9fin4c/);
...
test('rewards are chips only — non-withdrawable; no Stars/chain top-up on activity', () => {
  ...
  assert.match(lia, /play9fin4c/);
  assert.match(lia, /activity-entry-btn/);
  assert.match(lia, /activity-page/);
});

test('docs/qa play9fin4c present', () => {
  assert.equal(existsSync(join(root, '../../docs/qa/play9fin4c/README.md')), true);
  assert.equal(existsSync(join(root, '../../docs/qa/play9fin4c/report.json')), true);
});
