/**
 * play9ui2b — lobby activity / daily supply entry
...
test('cache play9ui2b', () => {
  ...
  assert.match(html, /lobby-ia\.css\?v=play9ui2b/);
...
test('rewards are chips only — non-withdrawable; no Stars/chain top-up on activity', () => {
  ...
  assert.match(lia, /play9ui2b/);
  assert.match(lia, /activity-entry-btn/);
  assert.match(lia, /activity-page/);
});

test('docs/qa play9ui2b present', () => {
  assert.equal(existsSync(join(root, '../../docs/qa/play9ui2b/README.md')), true);
  assert.equal(existsSync(join(root, '../../docs/qa/play9ui2b/report.json')), true);
});
