/**
 * play9ui1a — lobby activity / daily supply entry
...
test('cache play9ui1a', () => {
  ...
  assert.match(html, /lobby-ia\.css\?v=play9ui1a/);
...
test('rewards are chips only — non-withdrawable; no Stars/chain top-up on activity', () => {
  ...
  assert.match(lia, /play9ui1a/);
  assert.match(lia, /activity-entry-btn/);
  assert.match(lia, /activity-page/);
});

test('docs/qa play9ui1a present', () => {
  assert.equal(existsSync(join(root, '../../docs/qa/play9ui1a/README.md')), true);
  assert.equal(existsSync(join(root, '../../docs/qa/play9ui1a/report.json')), true);
});
