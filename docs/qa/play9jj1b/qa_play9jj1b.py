#!/usr/bin/env python3
"""play9jj1b: JJ landscape letterbox stage (no rotate) + settle HUD + play regress"""
import json, time, hashlib, os, subprocess, sys
from pathlib import Path
from playwright.sync_api import sync_playwright

OUT = Path('/workspace/play9u2-repo/docs/qa/play9jj1b')
OUT.mkdir(parents=True, exist_ok=True)
PORT = int(os.environ.get('PORT', '5231'))
URL = f'http://127.0.0.1:{PORT}/index.html?v=play9jj1b'
...
REPORT = {
  ...
  'ok': False,
  'root_causes': [
    'table-play position:fixed + vw/vh + safe-area on .self-slot fought letterbox → 跑偏',
    'play9jj1b: pin zones absolute-to-stage; top HUD flex band; L/R same top%; no vw on play-zone',
    'Landscape stage (W>H) letterbox translate+scale only — never rotate',
    'Kept play9v3g/v3h touch select + legal play + AI think + MATCH',
  ],
}
  ...
  return page.evaluate("""() => {
    ...
    return {
      ...
      zones: (() => {
        ...
        const actions = document.querySelector('.qq-center-actions, #playControls');
        const hud = document.querySelector('.qq-hud');
        const back = document.getElementById('backToLobbyButton');
        const more = document.getElementById('hudMoreToggle');
        ...
        const hdr = hud?.getBoundingClientRect();
        const bkr = back?.getBoundingClientRect();
        const mor = more?.getBoundingClientRect();
        const stageTop = r?.top || 0;
        const stageH = r?.height || 1;
        const stageL = r?.left || 0;
        const stageW = r?.width || 1;
        const midY = (a, b) => Math.abs(((a?.top||0)+(a?.height||0)/2) - ((b?.top||0)+(b?.height||0)/2));
        const seatTopDelta = (lr && rr) ? Math.abs(lr.top - rr.top) : 999;
        const firstCard = hand?.querySelector('.playing-card');
        const fcr = firstCard?.getBoundingClientRect();
        const avatarCoversFirst = Boolean(br && fcr && br.right > fcr.left + 8 && br.bottom > fcr.top + 10);
        const selfPos = selfBar ? getComputedStyle(selfBar).position : '';
        const slotPos = document.querySelector('.self-slot') ? getComputedStyle(document.querySelector('.self-slot')).position : '';
        return {
          holeTop: hol ? hol.top < stageTop + stageH * 0.28 : false,
          handBottom: hr ? hr.bottom > stageTop + stageH * 0.55 : false,
          leftOk: lr ? lr.left < stageL + stageW * 0.35 : false,
          rightOk: rr ? rr.right > stageL + stageW * 0.65 : false,
          meterVisible: meter ? getComputedStyle(meter).display !== 'none' : false,
          selfAvatar: br ? br.left < stageL + stageW * 0.25 : false,
          seatsAligned: seatTopDelta <= 12,
          seatTopDelta: Math.round(seatTopDelta),
          topBandAligned: Boolean(bkr && mor && hdr && Math.abs(bkr.top - mor.top) <= 14 && bkr.top >= stageTop - 2 && bkr.top < stageTop + 70),
          holeMeterBand: Boolean(hol && mr && Math.abs(hol.top - mr.top) <= 20 && hol.top < stageTop + 70),
          actionCentered: ar ? Math.abs((ar.left + ar.width/2) - (stageL + stageW/2)) <= stageW * 0.12 : true,
          handCentered: hr ? Math.abs((hr.left + hr.width/2) - (stageL + stageW/2)) <= stageW * 0.18 : false,
          avatarNotCoverFirst: !avatarCoversFirst,
          selfSlotAbsolute: slotPos === 'absolute',
          bottomBarAbsolute: selfPos === 'absolute',
          noSafePadDrift: (() => {
            const slot = document.querySelector('.self-slot');
            if (!slot) return false;
            const pb = getComputedStyle(slot).paddingBottom;
            return parseFloat(pb) <= 12;
          })(),
        };
      })(),
...
zones_ok = bool(zones.get('handBottom') and zones.get('leftOk') and zones.get('rightOk')
                 and zones.get('meterVisible') and zones.get('selfAvatar'))
align_ok = bool(zones.get('seatsAligned') and zones.get('selfSlotAbsolute')
                and zones.get('bottomBarAbsolute') and zones.get('noSafePadDrift')
                and zones.get('avatarNotCoverFirst') and zones.get('actionCentered')
                and zones.get('handCentered') and zones.get('topBandAligned'))

REPORT['pass'] = {
  'landscape_stage_w_gt_h_no_rotate': land_ok,
  'portrait_letterbox_no_rotate': port_ok and port_letterbox,
  'jj_zones': zones_ok,
  'zone_alignment_locked': align_ok,
