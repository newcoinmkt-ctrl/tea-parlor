#!/usr/bin/env python3
"""play9v2: JJ fan hands + hard-fix select/play @ 414"""
import json, time, hashlib
from pathlib import Path
from playwright.sync_api import sync_playwright

OUT = Path('/workspace/play9u2-repo/docs/qa/play9v2')
OUT.mkdir(parents=True, exist_ok=True)
URL = 'http://127.0.0.1:5193/index.html?v=play9v2'
VW, VH = 414, 896
REPORT = {
  'cache': 'play9v2',
  'viewport': [VW, VH],
...
with sync_playwright() as p:
  ...
  # Select via real pointer + API fallback
  sel = page.evaluate("""() => {
    const g = window.__teaParlor?.state?.()?.game;
    ...
    // Prefer API toggle with first card id
    ...
    cards[0].click();
    // If still none, force class + selected set via toggleCard again
  ...
  REPORT['rootCause'] = {
    'selectPlay': 'canSelectHand requires phase=play; play9v1 QA clicked during bid. Also toggleHandCard did not enable #playButton — fixed via syncPlayButtonFromSelection + forceDdzPlay + pointer-events on fan/HUD.',
  ...
  REPORT['pass'] = {
    ...
    'ddz_selected_gt0': (sel.get('selected') or 0) > 0,
