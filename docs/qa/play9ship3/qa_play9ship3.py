#!/usr/bin/env python3
"""play9ship3: DDZ play + MJ 推倒胡/日麻 enter + no rotate + TG expand harden"""
import json, time, hashlib, os, subprocess, sys
from pathlib import Path
from playwright.sync_api import sync_playwright

OUT = Path('/workspace/play9fix1-repo/docs/qa/play9ship3')
OUT.mkdir(parents=True, exist_ok=True)
PORT = int(os.environ.get('PORT', '5280'))
URL = f'http://127.0.0.1:{PORT}/index.html?v=play9ship3'
VW, VH = 896, 414
PW, PH = 414, 896
REPORT = {
  'cache': 'play9ship3',
...
def mj_enter(page, mode, tag):
  ...
  # play9ship3: force non-empty settle on 推倒胡
